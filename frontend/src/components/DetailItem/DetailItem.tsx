...
//   const [machine, setMachineId] = useState<string>("");
...
//     const [, machine, machineid, sensor, id] = pathname.split("/");
...
const DetailItem: React.FC = () => {
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      ...
      const newEntry: any = { x: currentTime };
      for (let i = 1; i <= 1; i++) {
        const randomValue = faker.datatype.number({ min: 10, max: 290 });
        newEntry[`${sensor}/${id}`] = randomValue;

        // axios를 통해 받아온 데이터로 대체
        if (historydata.length < 10) {
          newEntry[`${sensor}/${id}`] =
            historydata[historydata.length - 1]?.y ?? randomValue;
        } else {
          newEntry[`${sensor}/${id}`] = historydata[i].y;
        }
      }
      setData((prevData) =>
        prevData.length >= 10
